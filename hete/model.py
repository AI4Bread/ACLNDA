...
def contrastive_loss(projected_emd, v_emd, sampled_embeddings_u, sampled_embeddings_neg_v, tau, lambda_loss):
    ...
    neg_score = torch.log(
        pos + torch.sum(torch.exp(torch.bmm(v_emd, sampled_embeddings_neg_v.transpose(-1, -2)).squeeze() / tau),
                        dim=1).unsqueeze(-1))
...
def wo_loss(predicted_emd, neighbor_emd):

    # 计算预测节点与其邻居节点间的欧氏距离平方
    dist_squared = torch.sum((predicted_emd - neighbor_emd) ** 2, dim=-1)
    # 对每个节点的邻居节点间的距离平方和进行平均
    loss_per_node = torch.mean(dist_squared, dim=-1)
    # 对所有节点的损失进行平均
    total_loss = torch.mean(loss_per_node)
...
class ACLNDA(nn.Module):
    def __init__(self, in_dim, hidden_dim, layer_num, sample_size, tau, norm_mode="PN-SCS", norm_scale=20,
                 lambda_loss=1, moving_average_decay=0.0, num_MLP=3):
        super(ACLNDA, self).__init__()
    ...
    def reconstruction_neighbors(self, projected_emd, u_emd, v_emd, neighbor_dict):
        sampled_embeddings_u, sampled_embeddings_neg_v = self.sample_neighbors(neighbor_dict, u_emd,
                                                                               normalize(v_emd, p=2, dim=-1))
        projected_emd = projected_emd.unsqueeze(1)
        neighbor_recons_loss = contrastive_loss(projected_emd, normalize(v_emd, p=2, dim=-1).unsqueeze(1),
